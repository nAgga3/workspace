...
function getOneTimeUrl(serverurl,apikey,model,virtualpath,connectionName,passlen,expires,callback){function getCustomSettingsParams(){var customSettingsParams={};var paramsCollection=document.getElementsByTagName("select");for(var n=0;n<paramsCollection.length;n++){var sel=paramsCollection[n];if(sel.value!="none"){var aParam=sel.id.split("_");if(aParam.length==1)customSettingsParams[sel.id]=sel.value=="true";else{var root={};var fo=root;for(var pp=1;pp<aParam.length-1;pp++)fo[aParam[pp]]=fo={};fo[aParam[aParam.length-
1]]=sel.value=="true";customSettingsParams[aParam[0]]=root}}}return customSettingsParams}if(!serverurl)serverurl=location.pathname.substr(0,location.pathname.lastIndexOf("/"));if(!passlen)passlen=8;if(!expires)expires=30;if(serverurl.lastIndexOf("/")==serverurl.length-1)serverurl=serverurl.substring(0,serverurl.length-1);var cd=serverurl!="";var dt=helper.ajax.responseType.TEXT;var curdate=new Date;var currtime=curdate.getFullYear()+"-"+curdate.getMonth()+"-"+curdate.getDate()+"-"+curdate.getHours()+
"-"+curdate.getMinutes()+"-"+curdate.getSeconds()+"-"+curdate.getMilliseconds();var ajaxTimeout=6E4;helper.ajax.get({"url":serverurl+"/ws/oturl/get?apikey="+apikey+"&model="+model+"&virtualpath="+virtualpath+"&name="+connectionName+"&plen="+passlen+"&expires="+expires+"&t="+currtime,"dataType":dt,"cache":false,"withCredentials":!cd,"timeout":ajaxTimeout,"success":function(data){var url=data.url||data.URL||data;var csp=getCustomSettingsParams();if(Object.keys(csp).length!=0)url=url+"&csp="+encodeURIComponent(JSON.stringify(csp));
if(url.indexOf(serverurl)==-1)url=serverurl+url;if(callback)callback(url);else location.href=url},"error":function(e,status,xhr){alert(e&&e.message?e.message:xhr&&xhr.statusText?xhr.statusText:"")}})}
function generateOTURL(){getOneTimeUrl(document.getElementById("server").value,document.getElementById("apikey").value,document.getElementById("model").value,document.getElementById("virtualPath").value,document.getElementById("connectionName").value,document.getElementById("passlen").value,document.getElementById("expiration").value,function(url){console.log(url);document.getElementById("txtOtUrl").value=url;return})}
function connectToOTURL(){var url=document.getElementById("txtOtUrl").value;if(url!="")location.href=url}helper.dom.ready(function(){document.getElementById("btnGenerateOTURL").onclick=generateOTURL;document.getElementById("btnConnectToOTURL").onclick=connectToOTURL});
