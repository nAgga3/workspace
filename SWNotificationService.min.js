...
return new Promise(function(r,e){n[s]=function(t){if(o)if(o=!1,t)e(t);else{for(var i=Array(arguments.length-1),n=0;n<i.length;)i[n++]=arguments[n];r.apply(null,i)}};try{t.apply(i||null,n)}catch(t){o&&(o=!1,e(t))}})}},{}],2:[function(t,i,n){n.length=function(t){var i=t.length;if(!i)return 0;for(var n=0;1<--i%4&&"="==(t[0|i]||"");)++n;return Math.ceil(3*t.length)/4-n};for(var h=Array(64),f=Array(123),r=0;r<64;)f[h[r]=r<26?r+65:r<52?r+71:r<62?r-4:r-59|43]=r++;n.encode=function(t,i,n){for(var r,e=null,
s=[],o=0,u=0;i<n;){var f=t[i++];switch(u){case 0:s[o++]=h[f>>2],r=(3&f)<<4,u=1;break;case 1:s[o++]=h[r|f>>4],r=(15&f)<<2,u=2;break;case 2:s[o++]=h[r|f>>6],s[o++]=h[63&f],u=0}8191<o&&((e=e||[]).push(String.fromCharCode.apply(String,s)),o=0)}return u&&(s[o++]=h[r],s[o++]=61,1===u&&(s[o++]=61)),e?(o&&e.push(String.fromCharCode.apply(String,s.slice(0,o))),e.join("")):String.fromCharCode.apply(String,s.slice(0,o))};var a="invalid encoding";n.decode=function(t,i,n){for(var r,e=n,s=0,o=0;o<t.length;){var u=
t.charCodeAt(o++);if(61==u&&1<s)break;if((u=f[u])===nt)throw Error(a);switch(s){case 0:r=u,s=1;break;case 1:i[n++]=r<<2|(48&u)>>4,r=u,s=2;break;case 2:i[n++]=(15&r)<<4|(60&u)>>2,r=u,s=3;break;case 3:i[n++]=(3&r)<<6|u,s=0}}if(1===s)throw Error(a);return n-e},n.test=function(t){return/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(t)}},{}],3:[function(t,i,n){function c(i,n){"string"==typeof i&&(n=i,i=nt);var f=[];function h(t){if("string"!=typeof t){var i=a();if(c.verbose&&console.log("codegen: "+
i),i="return "+i,t){for(var n=Object.keys(t),r=Array(n.length+1),e=Array(n.length),s=0;s<n.length;)r[s]=n[s],e[s]=t[n[s++]];return r[s]=i,Function.apply(null,r).apply(null,e)}return Function(i)()}for(var o=Array(arguments.length-1),u=0;u<o.length;)o[u]=arguments[++u];if(u=0,t=t.replace(/%([%dfijs])/g,function(t,i){var n=o[u++];switch(i){case "d":case "f":return""+ +(""+n);case "i":return""+Math.floor(n);case "j":return JSON.stringify(n);case "s":return""+n}return"%"}),u!==o.length)throw Error("parameter count mismatch");
...
t[r++]}},s.prototype.bytes=function(t){var i=(t=e.isString(t)?e.y(t,"base64"):t).length>>>0;return this.uint32(i),i&&this.g(s.writeBytesBuffer,i,t),this},s.prototype.string=function(t){var i=e.Buffer.byteLength(t);return this.uint32(i),i&&this.g(o,i,t),this},s.u()},{39:39,42:42}]},{},[19])}();var MessagingService=null;
(function(){var Root=protobuf.Root,Type=protobuf.Type,Field=protobuf.Field;var _noop=()=>{return null};var ChannelCmd={"CMD_INTERNAL":0,"CMD_OPEN":1,"CMD_CLOSE":2,"CMD_LINK":3,"CMD_UNLINK":4,"CMD_NEW":5,"CMD_DATA":6,"CMD_EVENT":7,"CMD_STATUS":8};var ChannelStatus={"INITIALIZED":0,"CONNECTED":1,"V1_CONNECTED":2,"DISCONNECTED":3,"TERMINATED":4,"OPEN":5,"LINKED":6,"UNLINKED":7,"CLOSED":8,"PEER_NEEDED":10};var ChannelError={"SUCCESS":0,"NOT_INITIALIZED":-1,"CHANNEL_EXISTS":-2,"INVALID_CHANNEL":-3,"READ_HEADER":-4,
"TIMEOUT":-5,"CHANNEL_INACTIVE":-6,"INVALID_DATA":-7,"APPLICATION_ERROR":-255};var ChannelOrigin={"CLIENT":0,"SERVER":1};var ChannelRunMode={"NODE":0,"HOOK":1};var globalPacketNumber=1;let _wsIsAvailable=false;let _connected=false;let _basePath="<%=@BASEURL%>";let _channel="__messaging__";let _events={};let _watcher={available:true,checkTime:3E3,echoTime:3E4,maxTime:6E4,intervalId:null,lastContactTime:0,currentTime:0,reconnectTime:5E3,echoGuid:"ECHO::"};const _prefixClassMsg="Messaging service - ";
const _wsStates=Object.freeze({"CONNECTING":0,"OPEN":1,"CLOSING":2,"CLOSED":3});function MessagingServiceClass(args){var _ref={};var _ws=null;function initialize(){try{if(WebSocket){_wsIsAvailable=true;attachEvents();reconnect(reconnect)}}catch(error){_wsIsAvailable=false;throw Error("Cannot initialize WebSocket connection. "+arguments.message);}}function initWS(callback){if(_wsIsAvailable&&!_ws){console.log(`${_prefixClassMsg}Initializing WebSocket`);let url=(location.protocol==="https:"?"wss":"ws")+
"://"+location.hostname+":"+location.port;url+=(_basePath==="/"?_basePath:location.pathname.split(_basePath)[0]+_basePath)+_channel;_ws=new WebSocket(url);_ws.binaryType="arraybuffer";_ws.addEventListener("open",()=>{fireEvent("open");console.log(`${_prefixClassMsg}WebSocket (Open)`);_connected=true;_watcher.lastContactTime=Date.now();startWatcher()});_ws.addEventListener("close",e=>{fireEvent("close");console.warn(`${_prefixClassMsg}WebSocket (Close) ${e.message||e}, reason: ${e.reason||"unknown"}, stack: ${e.stack||
""} `);_watcher.lastContactTime=0;startWatcher()});_ws.addEventListener("message",msg=>{_watcher.lastContactTime=Date.now();let decoded=decodeProtoBuffMsg(msg.data);if(decoded?.cmd!==_watcher.echoGuid)fireEvent("message",decoded)});_ws.addEventListener("error",e=>{fireEvent("error",e);console.warn(`${_prefixClassMsg}WebSocket (Error): ${e.message||e}, reason: ${e.reason||"unknown"}, stack: ${e.stack||""} `);_watcher.lastContactTime=0;startWatcher()});return true}else return false}function startWatcher(){stopWatcher();
if(_watcher.available)_watcher.intervalId=setInterval(()=>{let res=true;let d=Date.now();let dif=d-_watcher.lastContactTime;if(dif>_watcher.echoTime)res=sendEcho();if(!res||dif>_watcher.maxTime)reconnect(reconnect)},_watcher.checkTime)}function stopWatcher(){clearInterval(_watcher.intervalId);_watcher.intervalId=null;_watcher.echo=false}function sendEcho(){if(_ws&&_ws.readyState===_wsStates.OPEN){_watcher.echoGuid="ECHO::"+Date.now().toString();_ws.send((new TextEncoder("utf-8")).encode(JSON.stringify({cmd:_watcher.echoGuid})));
return true}else return false}function reconnect(callback){stopWatcher();if(_ws)_ws.close();_ws=null;if(!initWS()&&callback)setTimeout(()=>{callback()},_watcher.reconnectTime)}function attachEvents(){if(args?.events)Object.keys(args.events).forEach(k=>{onEvent(k,args.events[k])})}function decodeProtoBuffMsg(data){data=(new TextDecoder("utf-8")).decode(data);if(data)try{data=JSON.parse(data.substring(data.indexOf("{")))}catch(err){console.warn(`${_prefixClassMsg}Cannot process or decode packet. Message: ${err.message||
err}, Stack: ${err.stack||""}`);data=null}return data}function onEvent(name,handler){if(handler instanceof Function&&name){if(!(name instanceof Array))name=[name];name.forEach(n=>{if(!_events[n])_events[n]=[];_events[n].push(handler)})}}function offEvent(name,handler){if(handler instanceof Function&&name){if(!(name instanceof Array))name=[name];name.forEach(n=>{if(_events[n])for(var i=_events[n].length-1;i>=0;i--)if(_events[n][i]===handler)_events[n].splice(i,1)})}}function fireEvent(name,data){if(_events[name])try{_events[name].forEach(h=>
h(data))}catch(error){}}function write(message){_ws.send(message)}function dispose(){}initialize();Object.defineProperty(_ref,"on",{enumerable:true,configurable:false,writable:false,value:onEvent});Object.defineProperty(_ref,"off",{enumerable:true,configurable:false,writable:false,value:offEvent});Object.defineProperty(_ref,"write",{enumerable:true,configurable:false,writable:false,value:write});return _ref}MessagingService=MessagingServiceClass})();
(function(){const _basePath="<%=@BASEURL%>";const _minFiles=this.location?.pathname?.includes(".min")||false;let _commonScriptsLocation=`${_basePath}common/`;let _scriptsFiles=!_minFiles?["plugin/protobuf/protobuf.min.js","js/thinfinity.messaging.service.js"]:[];let _messagingSvc;let _registeredCmds={};let _ports=[];let _initializing=false;let _initialized=false;let _resolves=[];async function postInitialize(){return new Promise(resolve=>{if(_initialized)resolve();else if(_initializing)_resolves.push(resolve);
else{_initializing=true;try{_scriptsFiles.forEach((k,i)=>{_scriptsFiles[i]=_commonScriptsLocation+k});loadScripts(_scriptsFiles)}catch(err){console.warn(`Message: ${err.message}`)}registerCmds();initializeNotificationService().then(()=>{_initialized=true;resolve();_resolves.forEach(r=>r());_resolves=[]})}})}function registerCmds(){_registeredCmds["echo"]={incoming:data=>{sendMessage(data.message)},outgoing:data=>{sendMessage(data.message)}};_registeredCmds["messaging"]=data=>{sendMessage(data.message)};
_registeredCmds["notification"]=data=>{sendMessage(data.message)}}function loadScript(url){try{importScripts(url);return true}catch(err){console.warn(`Load failed: ${url}, reason: ${err.message}, stack: ${err.stack} `);throw new Error("Load scripts failed");}}function loadScripts(urls){urls=Array.isArray(urls)?urls:[urls];urls.forEach(f=>{loadScript(f)})}function sendMessage(message){_messagingSvc.write((new TextEncoder("utf-8")).encode(JSON.stringify(message)))}function sendToPorts(msg){_ports.forEach(port=>
{try{port.postMessage(msg)}catch(err){console.log("Invalid JSON Object",err)}})}function processResponse(data){if(data&&data.byteLength){let msg=(new TextDecoder("utf-8")).decode(data);if(msg)sendToPorts(JSON.parse(msg))}else sendToPorts(data)}async function initializeNotificationService(){return new Promise(resolve=>{_messagingSvc=new MessagingService({events:{open:()=>{console.log("opened")},message:processResponse,close:()=>{console.log("closed")},error:err=>{console.warn("Error: "+err.message)}}});
resolve()})}function processMessages(e){let d=e.data||{};let cmd=d?.cmd||d?.name;if(_registeredCmds[cmd])_registeredCmds[cmd].incoming(e.data)}onconnect=async event=>{await postInitialize();let port=event.ports[0];port.onmessage=processMessages;_ports.push(port)}})();
