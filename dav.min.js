...
helper.dom.ready(()=>{let handlersAttached=false;let serverResponse=null;let asked=false;let saveEnabled=false;let apiUrl="__base__";const pingsController={time:15E3,url:"auth/ping",intervalId:null};let credentialDialog=document.getElementById("credentialsDialog");Thinfinity.Theme.apply({auto:true});Thinfinity.Popups.establishing.backgroundStyle=Thinfinity.Popups.establishing.backgroundStyles.NONE;Thinfinity.Popups.establishing.show(consts.pleaseWait,null,true,false);const getVariable=(name,fromData)=>
{if(Thinfinity?.global?.variables)if(fromData&&Thinfinity.global.variables.data){let d=JSON.parse(Thinfinity.global.variables.data);return d[name]}else return Thinfinity.global.variables[name];else return undefined};const tryServerURL=(virtualPath,success,fail)=>{doGetServerURL(virtualPath).then(res=>{try{if(res&&res.redirect&&res.poolid)success.call(this,res);else tryServerURL(virtualPath,success,fail)}catch(err){tryServerURL(virtualPath,success,fail)}}).catch(err=>{fail.call(this)})};const resolveServerUrl=
virtualPath=>{return new Promise((resolve,reject)=>{tryServerURL(virtualPath,res=>{if(res.gateway)res.gateway=res.gateway.endsWith("/")?res.gateway.substr(0,res.gateway.length-1):res.gateway;res={...res,redirectUrl:`${res.gateway?res.gateway:""}/${res.poolid}/${virtualPath}${virtualPath!==""?"/":""}`,gateway:res.gateway??null};resolve(res)},reject)})};const doGetServerURL=vp=>{const url=`${typeof vp==="string"&&vp.length>0?`/${vp}/`:"/"}getServerUrl`;return fetch(url,{"referrerPolicy":"same-origin",
"body":null,"method":"GET","mode":"cors","credentials":"include"}).then(res=>res.json())};const keepConnectionAlive=()=>{clearInterval(pingsController.intervalId);pingsController.intervalId=setInterval(()=>{fetch(`${serverResponse.gateway}/${apiUrl}${pingsController.url}`.replaceAll("//","/")).catch(err=>{console.log("ping error: "+err.message)});fetch(`${serverResponse.gateway}/${serverResponse.accessKey}/${apiUrl}/${pingsController.url}`.replaceAll("//","/")).catch(err=>{console.log("ping ${} error: "+
err.message)})},pingsController.time)};function doLogin(data){return fetch(`${serverResponse.gateway}/${serverResponse.accessKey}/${apiUrl}/auth/login`,{"method":"post","referrerPolicy":"same-origin","mode":"cors","credentials":"include","body":JSON.stringify(data)})}function closeWithError(){let envData=environment.getEnvironmentInformation();if(envData?.webV7)window.parent.postMessage({to:"webV7",type:"connection",cmd:"close",meta:envData.webV7},"*");else{document.body.innerHTML="<div class='bye'>"+
consts.applicationClosed+"</div>";if(envData.inFullScreen&&envData.mainWindow&&envData.mainWindow.closeFullscreenContainer){envData.mainWindow.closeFullscreenContainer();return}else if(envData.mainWindow){let win=envData.mainWindow;if(envData.referrer!="")window.location.href=win.document.referrer;else if(envData.inNewWindow)win.close()}}}function attachHandlers(){helper.dom.addEvent(document.getElementById("signInBtn"),"click",submitUser);helper.dom.addEvent(document.getElementById("cancelSingInBtn"),
"click",cancelSubmit);helper.dom.addEvent(document.getElementById("credentialsDialog"),"keydown",function(e){switch(e.keyCode){case 13:submitUser();break;case 27:cancelSubmit();break}});handlersAttached=true}function submitUser(){helper.style.add(credentialDialog,"hidden");Thinfinity.Popups.unblockDialogs();Thinfinity.Popups.establishing.show(consts.pleaseWait,null,true,false);let sc=window?.customSettings?.noSaveCredentials||false;let se=saveEnabled&&!sc&&document.getElementById("saveCredentials").checked;
doLogin({"gwsid":getVariable("gwsid"),"urlkey":getVariable("profileKey",true),"username":document.getElementById("authusername").value,"password":document.getElementById("authpassword").value,"save":se,"origin":location.origin}).then(resolveLogin)}function cancelSubmit(){helper.style.add(credentialDialog,"hidden");Thinfinity.Popups.unblockDialogs();Thinfinity.Popups.alert(consts.invalidUserPass,consts.applicationClosed,"davAlertDlgError","","",closeWithError,false)}function resolveLogin(response){if(response)if(response.status===
200&&serverResponse?.redirectUrl){console.log(`Is ready to use.`);let oUrl=window.location.href;window.location.href=serverResponse.redirectUrl;window.history.pushState(null,undefined,oUrl)}else if(!serverResponse?.redirectUrl)Thinfinity.Popups.alert(consts.connectionNotStablished,consts.applicationClosed,"davAlertDlgError","","",closeWithError,false);else if(response.status===401)response.json().then(r=>{if(r.data.ask){keepConnectionAlive();askForCredentials(r.data)}else Thinfinity.Popups.alert(consts.invalidUserPass,
consts.applicationClosed,"davAlertDlgError","","",closeWithError,false)});else Thinfinity.Popups.alert(consts.connectionNotStablished,consts.applicationClosed,"davAlertDlgError","","",closeWithError,false)}function askForCredentials(data){if(!handlersAttached)attachHandlers();Thinfinity.Popups.unblockDialogs();Thinfinity.Popups.establishing.hide();let message=data.msg||"";if(asked)message=consts.invalidUserPass;let msgElem=document.getElementById("authErrMessage");saveEnabled=data?.saveEnabled;if(saveEnabled)helper.style.add(document.getElementById("saveCredRow"),
{"display":"table-row"});else helper.style.remove(document.getElementById("saveCredRow"),{"display":"none"});if(!message)helper.style.add(msgElem,"hidden");else{msgElem.innerHTML=message;helper.style.remove(msgElem,"hidden")}helper.style.remove(credentialDialog,"hidden");asked=true;setTimeout(()=>{document.getElementById("authusername").focus()},100)}function runConnection(){const vpath=location.pathname.replaceAll("/","");resolveServerUrl(vpath).then(res=>{if(res.redirectUrl){serverResponse={accessKey:res.poolid,
redirectUrl:res.redirectUrl,gateway:res.gateway||""};doLogin({"gwsid":getVariable("gwsid"),"urlkey":getVariable("profileKey",true),"username":"","password":"",save:false,"origin":location.origin}).then(resolveLogin)}else Thinfinity.Popups.alert(consts.connectionNotStablished,consts.applicationClosed,"davAlertDlgError","","",closeWithError,false)}).catch(err=>console.log("error: ",err))}runConnection()});
